...
class Read_HTML:
    ...
    def get_stock_info(self):
        ...
        try:
            ...
            #print('今日の株価のデータと株価トレンドのデータ、基本情報が取れませんでした\n上場廃止の可能性があります。', self.url)
            self.removed_code_list.append(self.code)
        try:
        ...
        try:  
            ...
            #print('業績推移のデータが取れませんでした、その他の情報は取得しています。', self.url)
...
class Read_HTML_FINANCE:
    ...
    def table_name(self, tables, finance):
        ...
        None_list = [None for _ in range(4)]
        for i in range(len(tables)):
            
            if cap1s[i] == '過去最高 【実績】':
                None_list[0] = self.table_to_pandas(tables[i])
            elif cap1s[i] == '通期':
                None_list[1] = self.table_to_pandas(tables[i])
            elif cap1s[i] == '第１四半期累計決算【実績】' or cap1s[i] == '第２四半期累計決算【実績】' or cap1s[i] == '前期【実績】' or cap1s[i] == '第３四半期累計決算【実績】':
                None_list[2] = self.table_to_pandas(tables[i])
            elif cap1s[i] == '財務 【実績】':
                None_list[3] = self.table_to_pandas(tables[i])
            else:
...
class Create_DB:
    ...
    def create_db(self):
        ...
        sql = str('CREATE DATABASE {}_db'.format(self.co_name))
    ...
    def create_table(self, info_dict):
        cursor = self.conn.cursor()
        sql = 'USE {}_db'.format(self.co_name)
        cursor.execute(sql)
        #sql = "DROP TABLE IF EXISTS stock_info_1"
        #cursor.execute(sql)

        try:
           
            sql = """CREATE TABLE stock_info_1(企業名 VARCHAR(32),証券コード VARCHAR(10),株価 VARCHAR(10),変化 VARCHAR(10),
            ...
            #query = '''ALTER TABLE stock_info_1 RENAME COLUMN 日時 TO 日時（年月日）'''
            #cursor.execute(query, self.conn)
            #self.conn.commit
            sql = "INSERT IGNORE INTO stock_info_1 ({}予備) VALUES ({}NULL)"
            sql = sql.format('{},'*26, '\'{}\','*26)
            sql = sql.format(*list(info_dict.keys())[:-1], *list(info_dict.values())[:-1])
            cursor.execute(sql)
            self.conn.commit()

            return True

        '''cursor.execute('DESC stock_info_1')
...
class Save_Data(Read_HTML, Read_HTML_FINANCE):
    ...
    def __init__(self, url_1, url_2, code):
        ...
        if create_db ==False:
            ...
            print('During creating DB, some problem occured. ')
    ...
    def arrange_db(self, caption):
        ...
        sql = "USE {}_db".format(self.co_name)
        cursor.execute(sql)
        sql = 'SELECT * FROM {}'.format(caption)
        cursor.execute(sql)
        df = pd.read_sql(sql = sql, con=self.creater.conn)
